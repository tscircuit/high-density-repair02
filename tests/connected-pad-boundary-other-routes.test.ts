...
test("pad-side exemption does not disable other routes or other sides", () => {
  const sample: DatasetSample = {
    ...
    nodeHdRoutes: [
      {
        connectionName: "pad-net", connectedPadSides: ["bottom"],
        route: [{ x: -4, y: -5 }, { x: -3, y: -5 }, { x: -2, y: -5 }],
      },
      {
        connectionName: "other-net",
        route: [{ x: 2, y: -5 }, { x: 3, y: -5 }, { x: 4, y: -5 }],
      },
      {
        connectionName: "top-net", connectedPadSides: ["bottom"],
        route: [{ x: -1, y: 5 }, { x: 0, y: 5 }, { x: 1, y: 5 }],
      },
    ],
  }
  ...
  expect(routes[1]!.route!.slice(1, -1).some((point) => point.y > -5)).toBe(true)
