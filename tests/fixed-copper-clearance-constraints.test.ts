...
test("fixed copper uses physical width, net identity, and the complete via span", () => {
  const wire = (y: number, z = 2, root = "signal"): HdRoute => ({
    ...
    traceThickness: 0.1,
    route: [{ x: -1, y, z }, { x: 1, y, z }],
  })
  const fixed: HdRoute = {
    connectionName: "ground-via",
    rootConnectionName: "ground",
    viaDiameter: 0.3,
    route: [{ x: 0, y: 0, z: 3 }, { x: 0, y: 0, z: 1 }],
    vias: [{ x: 0, y: 0 }],
  }
  ...
  expect(guard.allows([wire(0.4, 2, "ground")], [wire(0, 2, "ground")], [0])).toBe(true)
  ...
  const movingVia = (y: number): HdRoute => ({ ...fixed, connectionName: "signal",
    rootConnectionName: "signal", route: [{ x: 0, y, z: 1 }, { x: 0, y, z: 3 }],
    vias: [{ x: 0, y }],
  })
